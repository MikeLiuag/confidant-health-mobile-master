...
import {addTestID, AlertUtil, getHeaderHeight, isIphoneX,  Colors, PrimaryButton, TextStyles, CommonStyles } from 'ch-mobile-shared';
...
class GroupDonationScreen extends Component<Props> {
    ...
    constructor(props) {
        ...
        const suggestedAmount = this.groupInfo?.groupDonationSettings?.suggestedAmount;
        const selectedSuggestionIndex = PAYMENT_SUGGESTIONS.findIndex(paymentSuggest=> paymentSuggest.amount === suggestedAmount)
        this.state = {
            isLoading: true,
            donationDetails: null,
            customAmount: suggestedAmount  ? suggestedAmount  : '10',
            selectedPaymentMeta: '',
            selectedPaymentType: '',
            selectedSuggestionIndex: suggestedAmount ? selectedSuggestionIndex : 2,
        };
    }
    ...
    renderPaymentSuggestions = () => {
        return <ScrollView showsHorizontalScrollIndicator={false}
                           ...
                           onLayout={() => {
                               setTimeout(() => {
                                   if (this.scrollView) {
                                       this.scrollView.scrollTo({x: 50, y: 0, animated: true});
                                   }
                               }, 10);
                           }}>
            {
                ...
                <Content contentContainerStyle={{paddingTop: 20}}>
                    <View style={styles.textBox}>
                        ...
                        <Text
                            ...
                            Thanks for participating in the {this.groupInfo.name}
                        </Text>
                        <Text
                            {...addTestID('Heading-2')}
                            style={styles.donationSubText}>
                            Everyone in Confidant contributes to help other members of the community.
                        </Text>
                    </View>
                    ...
                    <View style={{paddingHorizontal: 24}}>
                        ...
                        <Text
                            {...addTestID('Heading-3')}
                            style={styles.donationSubNoteText}>
                            100% of the money you pay for the group goes to clinical care for people they can't
                            currently afford it.
                        </Text>
                    </View>
...
export default connectPayment()(GroupDonationScreen);
